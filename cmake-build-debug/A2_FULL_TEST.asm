...
ADDS	AADD.B R0,A0	; A0.lsb <- R0.lsb <- 0x00AB

	MOVLZ #0,A1		; A1 <- 0x0000
	MOVLS #0,A2		; A2 <- 0xFF00
	AADD $32,A1		; A1 <- 0x0000 + 0x0020
	AADD #10,A2		; A2 <- 0xFF00 + 0x0010

	MOVL #34,SP	
	MOVH #1200,SP	; SP <- 0x1234
	MOVL #21,BP
	MOVH #4300,BP	; BP <- 0x4321
	AADD.W SP,BP	; BP <- BP + SP = 0x5555
...
SUBS	ASUB.B R0,A0	; A0.lsb <- A0.lsb - R0.lsb <- 0x000F

	MOVLZ #0,A1		; A1 <- 0x0000
	MOVLS #FF,A2	; A2 <- 0xFFFF
	ASUB $1,A1		; A1 <- 0x0000 - 0x0001 = 0xFFFF
	ASUB #10,A2		; A2 <- 0xFFFF - 0x0010 = 0xFFEF

	MOVL #34,SP	
	MOVH #1200,SP	; SP <- 0x1234
	MOVL #21,BP
	MOVH #4300,BP	; BP <- 0x4321
	ASUB.W SP,BP	; BP <- BP - SP = 0x30ED
